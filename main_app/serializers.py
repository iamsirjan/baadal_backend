...
class ProductSerializer(serializers.ModelSerializer):
    ...
    thumbnailimage = serializers.ImageField(max_length=None, use_url=True, required=False, validators=[])
    image1 = serializers.ImageField(max_length=None, use_url=True, required=False, validators=[])
    image2 = serializers.ImageField(max_length=None, use_url=True, required=False, validators=[])

    class Meta:
        model = Product
        fields = ['product_id', 'name', 'baseprice', 'finalprice', 'created_at', 'saletime', 'auctionday',
                  'saledate', 'saleend','description', 'age', 'status', 'product_image', 'user', 'thumbnailimage','image1','image2','verified','police_verified']
    ...
    def image1_url(self, obj):
        return self.context['request'].build_absolute_uri(obj.image1.url) if obj.image1 else ''
    

    def image2_url(self, obj):
        return self.context['request'].build_absolute_uri(obj.image2.url) if obj.image2 else ''
